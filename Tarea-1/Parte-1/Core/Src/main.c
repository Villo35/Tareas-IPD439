...
{
  /* USER CODE BEGIN 1 */
	uint8_t origen[array_size];
	uint8_t destino_dma[array_size];
	uint8_t destino_cpy[array_size];
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
	llenar_array(origen, array_size);

	hdma_memtomem_dma1_channel1.XferCpltCallback = &DmaXferCompleteCallback;
